...
parser.add_argument("--data_path", type=str, default="/data0/datasets/spider/database_csv")
...
parser.add_argument("--model_path", type=str, default='checkpoints_save/encoder-contrastive-spider+pretrain-100epochs-0.0001lr.pt')
...
def generate_table_embedding(args):
    sentence_model = SentenceTransformer("/data0/pretrained-models/paraphrase-MiniLM-L6-v2")
    ...
    for table_emb, table_mask, table_path in tqdm(dataloader):
        ...
        with torch.no_grad():
            ...
            for i, path in enumerate(table_path):
                ...
                print(record.shape)
                dir_path = os.path.join("/data0/datasets/spider/database_emb", path.split("/")[-2])
